...
export default function MachinesPage() {
  ...
  function startEdit(m: MachineRow) {
    setEditingId(m.id);
    setFormName(m.name);
    setFormNote(m.note ?? "");
    setFormColor(m.color);
    setFormOrder(m.sortOrder);
  }

  async function submit() {
    const name = formName.trim();
    if (!name) {
      alert("Tên máy không được trống");
      return;
    }
    const payload = {
      name,
  ...
  async function deleteMachine(id: number, count: number) {
    const msg =
      count > 0
        ? `Xóa máy này? ${count} tài khoản đang gán sẽ về "Chưa đặt".`
        : "Xóa máy này?";
    ...
    <>
      <header className="page-header">
        <div className="page-header-left">
          <span className="eyebrow">Máy login</span>
      ...
      <section className="section">
        ...
        >
          <div>
            ...
            <input
              ...
              placeholder="Ví dụ: Máy 01, PC-Win-A…"
              style={{ width: "100%" }}
            />
          </div>
          <div>
        ...
        <div style={{ marginTop: 20, display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button onClick={submit} className="btn btn-accent">
            {editingId == null ? "Tạo máy" : "Lưu thay đổi"}
          </button>
          {editingId != null && (
        ...
        <div className="section-head">
          <div>
            ...
            <div className="section-label" style={{ marginTop: 4 }}>
              {rows.length === 0
                ? "Chưa có máy nào"
                : `${rows.length} máy · ${rows.reduce((s, r) => s + r.count, 0)} tài khoản đã gán`}
            </div>
          </div>
              ...
              {rows.length === 0 && !loading && (
                <tr>
                  <td colSpan={6} style={{ padding: 56, textAlign: "center" }}>
                    ...
                    >
                      Tạo máy đầu tiên ở form phía trên
                    </div>
                  </td>
                </tr>
              )}
              {rows.map((m) => (
                <tr key={m.id} className={editingId === m.id ? "selected" : ""}>
                  <td>
                    <span
                      style={{
                        width: 20,
                  ...
                  <td>
                    {m.note ? (
                      <span style={{ fontSize: 12, color: "var(--ink-soft)" }}>{m.note}</span>
                    ) : (
                      <span className="text-muted">—</span>
                    )}
                  </td>
                  <td className="right">
                    <span className="mono-num text-muted">{m.sortOrder}</span>
                  </td>
                  <td className="right">
                    <span className="mono-num">{m.count}</span>
                  </td>
                  <td>
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button
                        onClick={() => deleteMachine(m.id, m.count)}
